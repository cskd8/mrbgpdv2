...
use crate::packets::header::{self, Header, MessageType};
...
impl OpenMessage {
    pub fn new(my_as_number: AutonomousSystemNumber, my_ip_addr: Ipv4Addr) -> Self {
        let header: Header = Header::new(29, MessageType::Open);
        Self {
            header,
            version: Version::new(),
            my_as_number,
...
impl TryFrom<BytesMut> for OpenMessage {
    ...
    fn try_from(bytes: BytesMut) -> Result<Self, Self::Error> {
        ...
        let my_as_number = AutonomousSystemNumber::from(u16::from_be_bytes(bytes[20..22].try_into().context(format!("bytes length is too short"))?));
        let hold_time: HoldTime = HoldTime::from(u16::from_be_bytes(bytes[22..24].try_into().context(format!("bytes length is too short"))?));
        let b: [u8; 4] = bytes[24..28].try_into().context(format!("bytes length is too short"))?;
        let bgp_identifier: Ipv4Addr = Ipv4Addr::from(b);
        let optional_parameter_length = bytes[28];
        let optional_parameters: BytesMut = BytesMut::from(&bytes[29..]);
...
mod tests {
    ...
    fn convert_bytes_to_open_message_and_open_message_to_bytes() {
        ...
        let open_message2 = open_message_bytes.try_into().unwrap();
